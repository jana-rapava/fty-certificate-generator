/*  =========================================================================
    fty_certificate_generator_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    ...
    { "certgen_storage_config", NULL, true, false, "certgen_storage_config_test" },
...
{
    test_item_t *item;
    printf ("Running fty-certificate-generator selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_CERTIFICATE_GENERATOR_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_certificate_generator_private_selftest (verbose, item->subtest);
#endif // FTY_CERTIFICATE_GENERATOR_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_certificate_generator_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-certificate-generator test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_CERTIFICATE_GENERATOR_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_certificate_generator_private_selftest (verbose, test->subtest);
#endif // FTY_CERTIFICATE_GENERATOR_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
